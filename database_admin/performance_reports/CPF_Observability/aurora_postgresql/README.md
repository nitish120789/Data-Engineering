...
## Purpose
Generate AWR-style deep performance diagnostics for **aurora_postgresql** with default **30-minute snapshots** and **7-day retention**.
...
The Aurora PostgreSQL report is structured in the same AWR-style layout used by SQL Server/MySQL: identity, configuration, pressure indicators, active workload, top SQL dimensions, contention, storage, and replication posture.

### Engine-Specific Requirements
- Client tools: psql
- Recommended privileges: pg_read_all_stats, access to pg_stat_statements, Aurora replication views (where available), pg_stat_replication, and pg_replication_slots
