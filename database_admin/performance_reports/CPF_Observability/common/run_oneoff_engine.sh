...
collect_postgresql_family() {
  ...
  append_section "WAL and Checkpoint Pressure" run_psql_sql "SELECT buffers_clean, maxwritten_clean, buffers_alloc, stats_reset FROM pg_stat_bgwriter; SELECT wal_records, wal_fpi, wal_bytes, wal_buffers_full, stats_reset FROM pg_stat_wal;"
